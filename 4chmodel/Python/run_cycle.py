...
def parser_commands():
    ...
    parser.add_argument('--AV_resistance',
            type = float,
            default  = 0.03, # https://link.springer.com/content/pdf/10.1007%2Fs10237-013-0523-y.pdf
            help = 'Aortic valve resistance. Corresponds to the serial resistor in the windkessel model. Units are mmHg/mL/s')
    parser.add_argument('--case',
                        default='01HC',
                        help='Pick mesh')
    parser.add_argument('--c_atria',
                        type = float,
                        default=7.45,
                        help = 'Stiffness parameter in the neohokean material model for the atria.')
    parser.add_argument('--c_veins',
                        type = float,
                        default = 7.45,
                        help = 'Stiffness parameter in the neohokean material model for the veins.')
    ...
    parser.add_argument('--mechDT',
                        type=float,
                        default=1.)
    parser.add_argument('--mesh_name_opt',
                        default = '',
                        help = 'Name of the mesh to read. Optional, if not given it reads it with the default name.')
    parser.add_argument('--PA_press_mmHg',
                        type = float,
                        default =  17.4, #Right ventricle dimensions and function in response to acute hypoxia in healthy human subjects
                        help = 'Pulmonary pressure during ED. Corresponds to the initial condition of the system. In mmHg')
    ...
    parser.add_argument('--PV_resistance',
                        type=float,
                        default= 0.015, #0.5*0.03 Scaled from Hyde al. Improvement of right ventricular... 2016
                        help = 'Pulmonary valve resistance. Corresponds to the serial resistor in the Windkessel model. Units are mmHg/mL/s',
                        )
    ...
    parser.add_argument('--t_dur',
                        type = float,
                        default = 550.0,
                        help='Duration of transient (t_max)')
    return parser
...
def run(args, job):
    ...
    mesh_dir, mesh_name = mesh_file(args.case,args.mesh_name_opt)
...
def set_cv_sys(args,mesh_dir):
    ...
    if args.experiment == 'wk3':
        vols += ['-CV_coupling', 0,
            ...
            '-cavities[0].state', -1,
            '-lv_wk3.name', 'Aorta',
            '-lv_wk3.R1', float(args.AV_resistance),
            ...
            '-cavities[1].p0_out', float(args.PA_press_mmHg),
            '-cavities[1].valve', 0,
            '-cavities[1].state', -1,
            '-rv_wk3.name', 'PA',
            '-rv_wk3.R1', float(args.PV_resistance),
            '-rv_wk3.R2', 0.16*0.63,
            '-rv_wk3.C', 4.1*5.16,
