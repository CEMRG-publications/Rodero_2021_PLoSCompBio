...
def main(args):
    ...
    outfile='PM_' + args.heart + '_ava.dat'
    ...
    datdir = basedir+'/BiV/UVC_ava/UVC'
    datname = os.path.join(datdir, 'COORDS_Z')
    UVC_original_name = basedir + '/BiV/UVC/UVC/COORDS_Z'
    vtxdir = basedir+'/BiV'
    vtxname = os.path.join(vtxdir, 'BiV')
    namenewdat = 'COORDS_Z_new.dat'

    # Creating UVCs

    os.system('/home/crg17/Desktop/scripts/4chmodel/sh/run_UVC_PV.sh -c ' + args.heart)

    # print('\n\n UVCs created! Check the vtk if it doesn\'t work.\n Merging with the other UVC...\n)
    
    print('Reading UVC...\n')
    UVCpts = read_dat(datname+'.dat')
    UVCoriginal = read_dat(UVC_original_name + '.dat')
    newUVC = UVCoriginal
    for i in range(len(UVCpts)):
      newUVC[i] = max(UVCpts[i],UVCoriginal[i])
    ...
    for i, elm in enumerate(elem):
        if ( elem[i,4] in pcatags ):
            ...
            UVCelem.append(0.0)
    print('Saving file...\n')
    np.savetxt(os.path.join(basedir, 'UVC_elem_ava.dat'), UVCelem, fmt='%.8f')
    ...
    cmd = '/home/common/bin/GlVTKConvert -m '+basename+' -e '+os.path.join(basedir, outfile)+' -e '+os.path.join(basedir, 'UVC_elem_ava.dat')+' -F bin -o '+basename+'_elem_dat_UVC_ava'
    os.system(str(cmd))
